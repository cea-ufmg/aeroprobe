/*************************************************** 
  BMP180 pressure & temperature sensor class
...
boolean BMP180::begin(uint8_t mode) {
  ...
  // Initialize the b5 temperature compensation variable
  triggerTemperatureMeasurement();
  delay(5);
  computeB5(readRawTemperature());
  
  return true;
}

void BMP180::computeB5(int32_t UT) {
  int32_t X1 = (UT - (int32_t)ac6) * ((int32_t)ac5) >> 15;
  int32_t X2 = ((int32_t)mc << 11) / (X1+(int32_t)md);
  b5 = X1 + X2;
}

void BMP180::triggerTemperatureMeasurement(void) {
  write8(BMP180_CONTROL, BMP180_READTEMPCMD);    
}

uint16_t BMP180::readRawTemperature(void) {
#if BMP180_DEBUG == 1
  Serial.print("Raw temp: "); Serial.println(read16(BMP180_TEMPDATA));
#endif
  return read16(BMP180_TEMPDATA);
}

void BMP180::triggerPressureMeasurement(void) {
...
int32_t BMP180::readTemperature(void) {
